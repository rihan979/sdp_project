...
void load_questions() {
    ...
    strcpy(question_bank[q_index].category, "Bangla Literature");
    q_index++;
    ...
    question_bank[q_index].correct_answer = 1;
    strcpy(question_bank[q_index].category, "Bangla Literaturei");
    q_index++;
...
int start_game() {
    ...
    for (int stage = 0; stage < 3; stage++) {
        ...
        // Update prize after completing stage
...
int play_stage(struct Player *player, int stage_num, int category_index) {
    ...
    for (int q = 0; q < stages[stage_num].questions_count; q++) {
        ...
        if (!ask_question(question_bank[question_index],
                         stages[stage_num].time_limit,
                         helpline_available, player)) {
            return 0; // Wrong answer or timeout
        }
    ...
    return 1; // Stage completed successfully
}
